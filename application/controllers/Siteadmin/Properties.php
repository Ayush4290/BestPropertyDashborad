...
public function updateBulkStatus() {
 ...
 public function editProperties() {
    ...
    $data['title'] = 'Property Edit'; 
     ...
     $data['projects'] = $this->AdminModel->getProjects(); // Fetch projects
...
// Agar clone_id 0 nahi hai to clone ka data lao
   if ($clone_id != 0) {
	...
	if($this->input->post('save')) {
	   ...
	   // $this->form_validation->set_rules('amenities[]', 'amenities','trim|required');
	    ...
	    if ($this->form_validation->run() != FALSE) { 
	       ...
	       if($verified_value){$verified = implode('~-~',$verified_value);}
	       else{ $verified=""; }
