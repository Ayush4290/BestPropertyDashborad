	...
	class Leads extends CI_Controller {
		...
		public function index() {
			...
			//if ($this->session->userdata('role') == 'Agent' || $agent != '') { 
			    	if (stristr($this->session->userdata('role'), 'Agent') || $agent != '') { 
...
public function Addleads() {
    ...
    'userid' => $userId,
    'message' => 'Lead: ' . $insertData['uName'] . ' (' . $insertData['mobile'] . ') added new lead',
...
$this->AdminModel->addDataInTable($notificationData, 'notification');
    ...
    if ($this->input->post('save')) {
        ...
        if ($this->form_validation->run() != FALSE) {
            ...
            if ($currentData->location != $updateData['preferred_location']) {
                $logContent['location'] = [
            ...
            //  Notification
            $assignedUsers = $this->AdminModel->getDataFromTableByField($id, 'assigned_leads', 'leadid');
            if (!empty($assignedUsers)) {
                foreach ($assignedUsers as $assign) {
                    $notifyData = array(
                        'userid' => $assign->userid,
                        'message' => 'Lead: ' . $currentData->uName . ' (' . $currentData->mobile . ') updated',
                        'notiKey' => 'buyers_' . $id,
                        'status' => 0,
                        'date' => date('Y-m-d H:i:s')
                    );
                    $this->AdminModel->addDataInTable($notifyData, 'notification');
                }
            }
    ...
    $leadData = $this->AdminModel->getDataFromTableByField($id, 'buyers', 'id');
    if (!empty($leadData)) {
        $lead = $leadData[0];

        // Check if property with same lead_id already exists
        $existingProperty = $this->AdminModel->getDataFromTableByField($lead->id, 'properties', 'lead_id');
        if (empty($existingProperty)) {
            ...
            $this->AdminModel->addDataInTable($propertyData, 'properties');
        }
            }
}
...
public function remove_unwanted_words($input){
    $unwantedWords = ["area", "good location in", "near", "main", "market", "road side",
     "or", "side", ".", "any", "and"];
...
public function get_recommended_properties($lead_data) {
    $property_type      = isset($lead_data['property_type']) ? strtolower(trim($lead_data['property_type'])) : null;
    $max_budget         = isset($lead_data['max_budget']) ? (float)$lead_data['max_budget'] : null;
    $preferred_location = isset($lead_data['preferred_location']) ? strtolower(trim($lead_data['preferred_location'])) : null;
    $status             = isset($lead_data['status']) ? strtolower(trim($lead_data['status'])) : null;
    $lead_description   = isset($lead_data['description']) ? strtolower(trim($lead_data['description'])) : '';
    ...
    if (!empty($lead_description) || $property_type || $max_budget || $preferred_location || $status) {
        if ($property_type) {
        ...
        if (!empty($filtered_location)) {
            $filtered_location = preg_replace('/[^a-zA-Z0-9\s]/', '', $filtered_location);
...
public function export_data()
{
    ...
    foreach ($leads as $lead) {
        ...
        if (isset($lead['leads_type']) && strtolower($lead['leads_type']) === 'buyer') {
            $lead_input = [
                'property_type'      => $lead['property_type'] ?? '',
                'max_budget'         => $lead['max_budget'] ?? $lead['budget'] ?? '',
                'preferred_location' => $lead['preferred_location'] ?? $lead['city'] ?? '',
                'status'             => $lead['status'] ?? $lead['property_for'] ?? '',
                'description'        => $lead['description'] ?? '',
            ];
...
public function export_leads() {
    ...
    if (stristr($this->session->userdata('role'), 'Sale Person')) {
        ...
        $leads = $this->AdminModel->getFilteredLeads($filters, $like, 'buyers', '*', 'id', 'desc');
    ...
    foreach ($leads as $row) {
        $csv_string .= '"' . implode('","', array_map('trim', array_values($row))) . "\"\n";
    }
